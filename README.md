...
## Что нового в 1.1.2-beta

- **Пакетный перевод сайта:** массовый автоперевод из нативного дерева ресурсов с независимым выбором опубликованных, неопубликованных, скрытых и удалённых ресурсов; после завершения предлагается очистить кэш.
- **DeepL:** endpoint Free/Pro выбирается по ключу с авторизацией через заголовок; ответы читаются из корректного поля `translations[].text`.
- **Надёжность перевода:** cURL-интеграции Google и DeepL проверяют HTTP-статусы и безопасно логируют ошибки; ошибки API останавливают пакетную обработку ресурсов и лексиконов и показываются пользователю.
